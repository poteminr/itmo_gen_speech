    ...
    def forward(self, x):
        ...
        if self.normalize_stft:
            if self.return_complex:
                spec = spec / (self.n_fft ** 0.5)
            else:
                spec = spec / (self.n_fft ** 0.5)
        
        if self.return_complex:
            magnitude = torch.abs(spec).pow(self.power)
        else:
            magnitude = torch.abs(torch.view_as_complex(spec)).pow(self.power)

        mel = torch.matmul(magnitude.transpose(1, 2), self.mel_fbanks.to(x.device)).transpose(1, 2)
